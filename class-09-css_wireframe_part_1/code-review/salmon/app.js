...
var handle_form_input = function (form_event) {
  /*
  1. Make a new object from the input data
    (name, min_customers, max_customers, avg_cookies_per_customer, open_time, close_time)
  2. Add the new table row to the bottom of the table
  3. Recalculate the final total row.
  */

  form_event.preventDefault();

  // event.target === snapshot of the form element
  ...
  var open_time = event.target.openTime.value;
  var close_time = event.target.closeTime.value;
  var average_per_cust = event.target.cookiesCust.value;

  var newStore = new Store(name, min_customers, max_customers, average_per_cust, open_time, close_time);
...
// store_form.addEventListener('click', function(){console.log('you clicked me');});

// store_form.addEventListener('mouseenter', function(nicholas_event){
//   console.log(nicholas_event);
// });




// Cannot read property 'addEventListener' of null ( look to the left of the dot, the parent is null)
